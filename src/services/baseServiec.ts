...
class BaseService<T extends BaseModel> {
  ...
  sortJson<R extends QueryBuilder<T, T[]>>(originalQuery: R, sorts: SortOpts[]): R {
    if (sorts?.length > 0) {
      try {
        sorts.forEach((item) => {
  ...
  filterJson<R extends QueryBuilder<T, T[]>>(originalQuery: R, filter: FilterOpts[]): R {
    if (filter?.length > 0) {
      try {
  ...
  builWithGraphFetchClause(ctx: GraphQLResolveInfo, fieldName: string[]): string {
    const selections = ctx.fieldNodes[0].selectionSet.selections
    const fields = selections.filter((s) => s.kind === 'Field').map((s) => (<FieldNode>s)?.name?.value)
  ...
  buildWhereClause<R extends QueryBuilder<T, T[]>>(qb: R, { field, operator, value }: FilterOpts): R {
